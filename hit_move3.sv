...
module	hit_move3	(	
					...
					input	 logic left,   //move Y Up   
					input	 logic right,      //toggle X   
					input  logic collision_monster,         //collision if smiley hits an object
					input  logic [3:0] HitEdgeCode,	//one bit per edge
					input	 logic enter,
					input  logic collision_protection,
...
const int	x_FRAME_LEFT	=	(SafetyMargin + 32)* FIXED_POINT_MULTIPLIER; 
const int	x_FRAME_RIGHT	=	(614 - SafetyMargin - OBJECT_WIDTH_X)* FIXED_POINT_MULTIPLIER; 
...
begin : fsm_sync_proc
	...
	if (resetN == 1'b0) begin 
		...
		Xposition <= 0  ; 
		Yposition <= 0   ;
		Xback <= (INITIAL_X*FIXED_POINT_MULTIPLIER)   ;
		hit_reg <= 16'b0 ;
		stop = 1;
		x <= 0;
		go <= 2'b00;
		...
		// keys direction change 
				...
				if (go == 2'b11) begin	//rizing edge 
					Yspeed <= -150; // toggle direction
					stop = 0;
					x <= 1;
				end
